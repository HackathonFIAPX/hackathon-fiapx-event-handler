...
jest.mock("../usecase/VideoUploaded/VideoUploadedUseCase");

describe("ProcessS3NotificationsController", () => {
    let controller: ProcessS3NotificationsController;
    let mockVideoUploadedUseCase: jest.Mocked<VideoUploadedUseCase>;

    beforeEach(() => {
        mockVideoUploadedUseCase = new VideoUploadedUseCase() as jest.Mocked<VideoUploadedUseCase>;
        controller = new ProcessS3NotificationsController(mockVideoUploadedUseCase);
        jest.clearAllMocks();
    });
    ...
    it("should process S3 notification and return a 200 response", async () => {
        ...
        mockVideoUploadedUseCase.execute.mockResolvedValue(undefined);

        const response = await controller.execute(request);

        expect(mockVideoUploadedUseCase.execute).toHaveBeenCalledWith({
            bucket: "test-bucket",
            key: "test-key",
        });
    ...
    it("should throw an error if S3 bucket or key information is missing", async () => {
        ...
        mockVideoUploadedUseCase.execute.mockRejectedValue(new TypeError("Cannot read properties of undefined (reading 'name')"));
    ...
    it("should handle errors from the use case", async () => {
        ...
        mockVideoUploadedUseCase.execute.mockRejectedValue(error);
